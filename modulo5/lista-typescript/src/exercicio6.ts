...
function listaDeCredores (lista:Array<Clientes>) {
  let debitosTotais: number
  let soma: number
  let saldo: Clientes[] = []
  lista.map((item)=>{
    debitosTotais = item.debitos.reduce((total, atual) => total + atual, 0);
    soma = item.saldoTotal - debitosTotais;
...
console.log(listaDeCredores(listaDeClientes))